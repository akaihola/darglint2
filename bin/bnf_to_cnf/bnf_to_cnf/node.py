...
class Node(object):
    ...
    def defines(self, value: Any) -> bool:
        ...
        for production in self.filter(lambda x: x.node_type == NodeType.PRODUCTION):
            has_annotation = len(production.children) > 1 and Node.is_annotations(
                production.children[1]
            )
            if has_annotation:
                assert production.children[
                    1
                ].value, "The productions must have a symbol value."
    ...
    def equals(self, other: Any) -> bool:
        ...
        if self.node_type in TERMINAL_NODES:
            ...
            return self.probability == other.probability and all(
                [x.equals(y) for x, y in zip(self.children, other.children)]
            )
        elif self.node_type in NONTERMINAL_NODES:
            return all([x.equals(y) for x, y in zip(self.children, other.children)])
        elif self.node_type == NodeType.IMPORTS:
    ...
    def from_lark_tree(self, tree: Union[Tree, Token]) -> "Node":
        ...
        if tree.data == "start":
            ...
            return Node(
                NodeType.SEQUENCE,
                children=list(map(Node.from_lark_tree, tree.children[first_child:])),
            ...
            return Node(
                NodeType.FILENAMES,
                children=[
                    Node(NodeType.FILENAME, value=filename) for filename in filenames
                ],
            )
        else:
            raise Exception(f'Unrecognized Lark type "{tree.data}".  Check grammar.')
    ...
    def to_python(self, start_symbol: Optional[str] = None) -> str:
        if self.node_type == NodeType.TERMINAL:
            ...
            has_annotation = len(self.children) > 1 and Node.is_annotations(
                self.children[0]
            )
            if has_annotation:
            ...
            return "[" + ", ".join([n.to_python() for n in self.children]) + "]"
            ...
            comment = f"# Generated on {datetime.datetime.now()}\n"
            name = "Grammar"
    ...
    def has_symbol(x: str) -> Callable[["Node"], bool]:
        def _inner(y: Node) -> bool:
            return exists(y.filter(lambda z: Node.is_symbol(z) and z.value == x))
    ...
    def to_dot(self) -> str:
        ...
        def _node_name(node: "Node") -> str:
            if node in name_lookup:
                ...
                raise Exception(f"Unrecognized node type {node.node_type}")
    ...
    def merge_annotations(self, other: "Node"):
        ...
        has_annotations = self.children and Node.is_annotations(self.children[0])
