...
from darglint2.parse.cyk import parse as cyk_parse
from darglint2.parse.grammars.google_long_description import LongDescriptionGrammar
from darglint2.parse.identifiers import NoqaIdentifier
from darglint2.parse.long_description import parse
from darglint2.utils import CykNodeUtils
...
class LongDescriptionParserTestCase(TestCase):
    ...
    def assertTokensEqual(self, token1, token2, msg=""):
        if token1 is None and token2 is None:
            ...
            self.fail("{} != {}: {}".format(token1, token2, msg))
        self.assertEqual(
            token1.token_type,
            token2.token_type,
            msg,
    ...
    def assertNodesEqual(self, node1, node2, msg=""):
        ...
        self.assertTokensEqual(node1.value, node2.value, msg)
    ...
    def test_parse_with_other_symbols(self):
        for symbol in ["Returns", "Yields", "Args", "(", ")"]:
    ...
    def test_cyk_noqa_matches(self):
        for raw in [
            ...
            recursive_noqa = self.get_identifier(recursive_node, NoqaIdentifier)
            ...
            self.assertNodesEqual(cyk_noqa, recursive_noqa, repr(raw))
