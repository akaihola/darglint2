...
from darglint2.lex import condense, lex
from darglint2.parse.google import parse
...
from darglint2.utils import ConfigurationContext, CykNodeUtils
...
class DocstringTestCase(TestCase):
    ...
    def test_parse_multiple_sections(self):
        sections = {
            ...
            "long-description": "\n".join(["A long description can appear anywhere."]),
            "yields-section": "\n".join(
                [
                    "Yields:",
                    "    A bunch of numbers.",
    ...
    def test_bare_noqa_can_be_parsed(self):
        docstring = "\n".join(
            ["The first line may have something, but others are missing.", "", "# noqa"]
        )
        node = parse(condense(lex(docstring)))
        self.assertTrue(CykNodeUtils.contains(node, "noqa"))
    ...
    def test_parse_line_which_looks_like_definition(self):
        ...
        node = parse(condense(lex("    Returns: Some value.\n")))
        self.assertTrue(node)
    ...
    def test_parse_line_without_type_but_with_parentheses(self):
        ...
        node = parse(
            condense(lex("    A list of items (of type T), which pass the given test"))
        )
        self.assertTrue(node)
    ...
    def test_parse_yields_section(self):
        ...
        node = parse(condense(lex("Yields:\n" "    Nodes in a stream.\n" "\n")))
        self.assertEqual(
            node.symbol,
            "yields-section",
        )
    ...
    def test_can_have_word_returns_in_description(self):
        tokens = condense(lex("\n".join(["Returns the sum of squares."])))
        node = parse(tokens)
        self.assertTrue(node is not None)
                ...
                lex(
                    "\n".join(
                        [
                            ...
                            "",
                            "Non-Standard:" "    Everything about this.",
                            "",
                            "Yields:",
                            "    Values to analyze.",
                        ]
    ...
    def test_type_can_have_indents(self):
        docstring = "\n".join(
            ["Test.", "", "Args:", "    input (a,       b): test", "", "# noqa: S001"]
        )
        tokens = condense(lex(docstring))
        node = parse(tokens)
        self.assertTrue(CykNodeUtils.contains(node, "arguments-section"))
    ...
    def test_parse_sole_argument_with_two_lines(self):
        ...
        values = {
            ArgumentIdentifier.extract(x) for x in annotation_lookup[ArgumentIdentifier]
        }
        self.assertEqual(
            values,
            {"x"},
