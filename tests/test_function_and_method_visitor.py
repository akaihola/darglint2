...
from darglint2.analysis.function_and_method_visitor import FunctionAndMethodVisitor
...
class FunctionAndMethodVisitorTests(TestCase):
    ...
    def assertFoundMethods(self, program, *args):
        ...
        self.assertEqual(
            sorted({method.name for method in visitor.methods}), sorted(args)
        )
        return visitor
    ...
    def assertFoundFunctions(self, program, *args):
        ...
        visitor = self.parse_and_visit(program)
        self.assertEqual(
            sorted({function.name for function in visitor.functions}), sorted(args)
        )
        return visitor
    ...
    def assertFoundProperties(self, program, *args):
        ...
        visitor = self.parse_and_visit(program)
        self.assertEqual(
            sorted({function.name for function in visitor.properties}), sorted(args)
        )
        return visitor
