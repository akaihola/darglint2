...
from darglint2.parse.numpy import parse
...
class NumpydocTests(TestCase):
    ...
    def assertContains(self, docstring, node_name, msg=""):
        self.assertTrue(
            ...
            msg
            or "Expected docstring to contain {} but it did not".format(
                node_name,
            ),
        )
    ...
    def assertHasIdentifier(self, docstring, identifier):
        self.assertTrue(len(CykNodeUtils.get_annotated(docstring, identifier)) > 0)
    ...
    def test_header_can_have_variable_length(self):
        for underline in ["-" * x for x in range(1, 15)]:
            raw_docstring = "\n".join(
                [
                    "Cry aloud.",
                    "",
    ...
    def test_single_parameter(self):
        parameter_descriptions = [
            "Something.",
            "A slightly longer description which " "can contain a colon: yes.",
            "A description over\n    two lines.",
    ...
    def test_multiple_parameters(self):
        ...
        raw_docstring = raw_docstring_pattern.format(
            "\n".join(["{}:\n    Something".format(name) for name in names])
        )
        tokens = condense(lex(raw_docstring))
        docstring = parse(tokens)
        self.assertContains(docstring, "arguments-section")
