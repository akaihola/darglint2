...
from darglint2.integrity_checker import IntegrityChecker
...
class IntegrityCheckerNumpyTestCase(TestCase):
    ...
    def test_runs_other_checks_on_private_arguments(self):
        ...
        self.assertTrue(
            isinstance(errors[0], EmptyDescriptionError), errors[0].__class__.__name__
        )
    ...
    @skip(
        "See Issue #69: https://github.com/terrencepreilly/darglint/issues/69#issuecomment-596273866"
    )  # noqa: E501
    def test_empty_description_error(self):
        ...
        for section, item in [("Parameters", "x")]:
            ...
            self.assertTrue(
                any([isinstance(error, EmptyDescriptionError) for error in errors]),
...
class IntegrityCheckerSphinxTestCase(TestCase):
    ...
    def test_underspecified_parameter_types(self):
        ...
        self.assertTrue(len(errors) <= 1, [(x.message()) for x in errors])
        if errors:
            self.assertTrue(
...
class IntegrityCheckerTestCase(TestCase):
    ...
    def test_extra_raises_added_to_error(self):
        ...
        checker = IntegrityChecker()
        checker.run_checks(functions[0])
        self.assertEqual(len(checker.errors), 1, checker.errors)
        error = checker.errors[0]
    ...
    def test_bare_return_doesnt_count_but_doesnt_count_against(self):
        ...
        without_return = program_template.format(
            reindent(
                r"""
            Print the inverse.
        ...
        with_return = program_template.format(
            reindent(
                r"""
            Print the inverse.
    ...
    def test_empty_type_section(self):
        ...
        self.assertEqual(len(errors), 1, [(x.message()) for x in errors])
        self.assertTrue(
            isinstance(errors[0], EmptyTypeError), errors[0].__class__.__name__
        )
...
class StrictnessTests(TestCase):
    ...
    def assertHasNoErrors(self, config, docstring):
        with ConfigurationContext(**config):
            ...
            errors = checker.errors
            self.assertEqual(len(errors), 0, [(x.message()) for x in errors])
