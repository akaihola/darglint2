...
class DocstringBaseMethodTests(TestCase):
        ...
        (
            ...
            "\n".join(
                [
                    ...
                    "    An exception for if it's " "        invalid.",
                    "Exception",
                    "    Seemingly at random.",
                    "",
                    "Yields",
        ...
        for google_doc, sphinx_doc, numpy_doc in self.equivalent_docstrings:
            for section in [
                ...
                self.assertEqual(
                    ...
                    "Sections differ for {} type.".format(section),
                )
    ...
    def test_type_and_name_always_associated(self):
        ...
        sphinx_params = [":param {}: An explanation".format(name) for name in names] + [
            ":type {}: {}".format(name, _type) for name, _type in zip(names, types)
        ]
        ...
        self.assertEqual(google_items, numpy_doc.get_items(Sections.ARGUMENTS_SECTION))
...
class DocstringForNumpyTest(TestCase):
    ...
    def _combine(
        ...
        while i < len(argtype_pairs) - 1:
            ...
            #   of a different type.
            while j < len(argtype_pairs) and argtype_pairs[i][1] == argtype_pairs[j][1]:
    ...
    def test_extract_simplest_raises_section(self):
        ...
        docstring = Docstring.from_numpy(raw_docstring)
        self.assertTrue(docstring.get_section(Sections.RAISES_SECTION) is not None)
    ...
    def test_arguments_section_with_breaks_in_lines_and_indents(self):
        ...
        docstring = Docstring.from_numpy(raw_docstring)
        self.assertTrue(docstring.get_section(Sections.ARGUMENTS_SECTION) is not None)
        self.assertEqual(
